...
impl Api {
    pub fn new(dm_conversation_id: &str) -> Self {
        Self {
    ...
    pub fn all(dm_conversation_id: &str) -> Self {
        Self {
            dm_conversation_id: dm_conversation_id.to_owned(),
    ...
    pub fn open(dm_conversation_id: &str) -> Self {
        Self {
            dm_conversation_id: dm_conversation_id.to_owned(),
            dm_event_fields: Some(DmEventFields::all()),
            expansions: Some(Expansions::all()),
    ...
    pub fn build(&self, authentication: &impl Authentication) -> RequestBuilder {
        ...
        let url = make_url(&self.twapi_options, &URL.replace(":dm_conversation_id", &self.dm_conversation_id));
        let builder = client
            .get(&url)
