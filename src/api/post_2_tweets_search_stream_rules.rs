...
impl Api {
    ...
    pub fn build(&self, authentication: &impl Authentication) -> RequestBuilder {
        ...
        if let Some(dry_run) = self.dry_run.as_ref() {
        ...
        let builder = client
            ...
            .json(&self.body)
        ;
        authentication.execute(apply_options(builder, &self.twapi_options), "POST", &url, &query_parameters.iter().map(|it| (it.0, it.1.as_str())).collect::<Vec<_>>())
    }
