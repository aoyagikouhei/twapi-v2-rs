...
async fn test_get_2_tweets_id_quote_tweets() -> Result<()> {
    ...
    let bearer_auth = BearerAuthentication::new(bearer_code);    let (res, _rate_limit) = execute_twitter::<serde_json::Value>(|| get_2_tweets_id_quote_tweets::Api::open("1617696866413719556").build(&bearer_auth), &None).await?;
