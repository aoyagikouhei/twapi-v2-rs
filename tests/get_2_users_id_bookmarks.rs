use anyhow::Result;
use twapi_v2::api::{execute_twitter, get_2_users_id_bookmarks, BearerAuthentication};
...
async fn test_get_2_users_id_bookmarks() -> Result<()> {
    ...
    let builder = get_2_users_id_bookmarks::Api::open("1660518823991336966").build(&bearer_auth);
    ...
    if let Some(next_token) = response.meta.unwrap().next_token {
        let builder = get_2_users_id_bookmarks::Api::open("1660518823991336966")
