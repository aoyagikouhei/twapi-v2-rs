use anyhow::Result;
use twapi_v2::api::{BearerAuthentication, execute_twitter, get_2_tweets_search_all};
...
async fn test_get_2_tweets_search_all() -> Result<()> {
    ...
    let bearer_auth = BearerAuthentication::new(academic_bearer_code);
        let (res, _rate_limit) = execute_twitter::<serde_json::Value>(|| get_2_tweets_search_all::Api::open("東京")
        .max_results(10)
