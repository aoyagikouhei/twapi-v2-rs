use anyhow::Result;
use twapi_v2::api::{execute_twitter, get_2_users_id_followed_lists};
...
async fn test_get_2_users_id_followed_lists() -> Result<()> {
    ...
    let builder =
        get_2_users_id_followed_lists::Api::all(&bearer_code, "1660518823991336966").build();
    ...
    if let Some(next_token) = response.meta.unwrap().next_token {
        let builder = get_2_users_id_followed_lists::Api::all(&bearer_code, "1660518823991336966")
