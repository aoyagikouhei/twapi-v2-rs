use anyhow::Result;
use twapi_v2::api::{BearerAuthentication, execute_twitter, post_2_users_id_muting};
...
async fn test_post_2_users_id_muting() -> Result<()> {
    ...
    let bearer_auth = BearerAuthentication::new(bearer_code);
        let (res, _rate_limit) = execute_twitter::<serde_json::Value>(|| post_2_users_id_muting::Api::new("1660518823991336966", body.clone()).build(&bearer_auth), &None).await?;
