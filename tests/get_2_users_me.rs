...
async fn test_get_2_users_me() -> Result<()> {
    ...
    let bearer_auth = BearerAuthentication::new(bearer_code);    let (res, rate_limit) = execute_twitter::<serde_json::Value>(|| get_2_users_me::Api::all().build(&bearer_auth)).await?;
