use anyhow::Result;
use twapi_v2::api::{
    BearerAuthentication, execute_twitter, get_2_dm_conversations_dm_conversation_id_dm_events,
};
...
async fn test_get_2_dm_conversations_dm_conversation_id_dm_events() -> Result<()> {
    ...
    let bearer_auth = BearerAuthentication::new(bearer_code);    let (res, _rate_limit) = execute_twitter::<serde_json::Value>(|| get_2_dm_conversations_dm_conversation_id_dm_events::Api::all(&dm_conversation_id)
