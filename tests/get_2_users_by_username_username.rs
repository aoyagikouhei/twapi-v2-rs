...
async fn test_get_2_users_by_username_username() -> Result<()> {
    ...
    let bearer_auth = BearerAuthentication::new(bearer_code);    let (res, _rate_limit) = execute_twitter::<serde_json::Value>(|| get_2_users_by_username_username::Api::open("barley_tea_0522").build(&bearer_auth)).await?;
