use anyhow::Result;
use twapi_v2::api::{
    delete_2_users_source_user_id_muting_target_user_id, execute_twitter, BearerAuthentication,
};
...
async fn test_delete_2_users_source_user_id_muting_target_user_id() -> Result<()> {
    ...
    let builder = delete_2_users_source_user_id_muting_target_user_id::Api::new(
        "1660518823991336966",
        &target_user_id,
    )
    .build(&bearer_auth);
