...
async fn test_get_2_compliance_jobs() -> Result<()> {
    ...
    let bearer_auth = BearerAuthentication::new(app_bearer_code);    let (res, _rate_limit) = execute_twitter::<serde_json::Value>(|| get_2_compliance_jobs::Api::new(get_2_compliance_jobs::Type::Users).build(&bearer_auth)).await?;
