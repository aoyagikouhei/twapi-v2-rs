use anyhow::Result;
use twapi_v2::api::{BearerAuthentication, execute_twitter, get_2_spaces_id_buyers};
...
async fn test_get_2_spaces_id_buyers() -> Result<()> {
    ...
    let bearer_auth = BearerAuthentication::new(bearer_code);
        let (res, _rate_limit) = execute_twitter::<serde_json::Value>(|| get_2_spaces_id_buyers::Api::all(&spaces_id).build(&bearer_auth), &None).await?;
