use anyhow::Result;
use twapi_v2::api::{delete_2_users_id_bookmarks_tweet_id, execute_twitter};
...
async fn test_delete_2_users_id_bookmarks_tweet_id() -> Result<()> {
    ...
    let builder =
        delete_2_users_id_bookmarks_tweet_id::Api::new(&bearer_code, "19522946", &tweet_id)
            .build();
    let (res, _rate_limit) = execute_twitter::<serde_json::Value>(builder).await?;
    println!("{}", serde_json::to_string(&res).unwrap());
    let response =
        serde_json::from_value::<delete_2_users_id_bookmarks_tweet_id::Response>(res)?;
