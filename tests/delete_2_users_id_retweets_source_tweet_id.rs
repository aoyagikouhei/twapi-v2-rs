use anyhow::Result;
use twapi_v2::api::{execute_twitter, delete_2_users_id_retweets_source_tweet_id};
...
async fn test_delete_2_users_id_retweets_source_tweet_id() -> Result<()> {
    ...
    let builder = delete_2_users_id_retweets_source_tweet_id::Api::new(&bearer_code, &me_id, &tweet_id).build();
    let (res, _rate_limit) = execute_twitter::<serde_json::Value>(builder).await?;
    println!("{}", serde_json::to_string(&res).unwrap());
    let response = serde_json::from_value::<delete_2_users_id_retweets_source_tweet_id::Response>(res)?;
